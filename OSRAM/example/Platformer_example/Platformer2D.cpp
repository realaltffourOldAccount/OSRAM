...
void Platformer2D::init()
{
	...
	m_Input = new Input(m_Window->getWindowHandler(), GLFW_CURSOR_NORMAL);
	...
	for (int i = 0; i < MAP_MULTI; i++)
	{
		...
		map._BackgroundTex = new Sprite2DTex(map._background);
		...
		map._GroundTex = new Sprite2DTex(map._ground);
	...
	m_Platformer = new Sprite2DTex(m_PlatformerDATA);
	...
	m_VirtualPlatformer = new Sprite2DTex(m_PlatformerDATA);
	...
	for (int i = 0; i < ENTITIES; i++)
	{
		...
		temp._spriteTex = new Sprite2DTex(temp._data);
