...
void OSRAM::GRAPHICS::Sprite2DTex::init()
{
	...
	std::cout << "[TEXTURE] Loading File: " + _TexSpec._texturePATH << " . . . " << std::endl;
	stbi_set_flip_vertically_on_load(true);
	t_data = stbi_load(_TexSpec._texturePATH.c_str(), &t_Width, &t_Hieght, &t_nrChannels, 0);
	if (t_data == NULL)
		std::cout << "[TEXTURE] Loading File: " + _TexSpec._texturePATH << " . . . FAIL" << std::endl;
	else
		std::cout << "[TEXTURE] Loading File: " + _TexSpec._texturePATH << " . . . SUCCESS" << std::endl;
